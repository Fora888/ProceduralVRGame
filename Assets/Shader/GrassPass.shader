Shader "GrassPass" {
    ...
    SubShader{
        ...
        Pass {
            ...
            static const double DOUBLE_UNIT = 1.0 / (2147483647 + 1.0);
            ...
            void geo(triangle t2g input[3], inout TriangleStream<g2f> triStream)
            {
                ...
                for (float i = 0; i < _GrassBlades * LOD; i++) {
                    ...
                    float2 uv = (midpoint.xz  + _WindVector * _Time.y)* _WindDistortionMap_ST.xy + _WindDistortionMap_ST.zw;
                    float windSample = ((tex2Dlod(_WindDistortionMap, float4(uv, 0, 0)).x)) * _WindStrength;
                    float3 wind = normalize(float3(_WindVector.y, 0, -_WindVector.x));
                    ...
                    //Base without wind rotation
                    o.color = lerp(_ColorBase.xyz, _ColorTip.xyz, 0);
                    vertexPositions = GetVertexPositionInputs(mul(mul(float3(_GrassWidth, 0, 0) * r5, randomRotation), alignToNormal) + midpoint.xyz);
                    ...
                    //middle parts
                    for (uint i = 0; i < (uint)(_GrassSegments * LOD); i++)
                    {
                        float segmentMultiplier = (i + 1)  / (float)_GrassSegments;
                        o.color = lerp(_ColorBase.xyz, _ColorTip.xyz, segmentMultiplier);
                        vertexPositions = GetVertexPositionInputs(mul(normalVector * pow(segmentMultiplier, 3) * _Curvature * r4 + mul(mul(float3(_GrassWidth * (1 - segmentMultiplier), _GrassHeight * segmentMultiplier, 0) * r5, randomRotation), alignToNormal), windRotation) + midpoint.xyz);
                        o.pos = vertexPositions.positionCS;
                        o.posWS = vertexPositions.positionWS;
                        triStream.Append(o);
                    ...
                    vertexPositions = GetVertexPositionInputs(mul(normalVector * pow(1,3) * _Curvature * r4 + mul(mul(float3(0, _GrassHeight * r5, 0), randomRotation), alignToNormal), windRotation) + midpoint.xyz);
                    o.pos = vertexPositions.positionCS;
                    o.posWS = vertexPositions.positionWS;
                    triStream.Append(o);
            ...
            float4 frag(g2f i) : SV_Target
            {
                ...
                BRDFData brdfData;
                float alpha = 1;
                InitializeBRDFData(i.color.xyz, 0, 0, 0, alpha, brdfData);
